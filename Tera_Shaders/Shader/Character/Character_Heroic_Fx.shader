Shader "TERA/Character/Heroic_withFX" {
    Properties {
		...
		[Header(displacement flow fx)]
		[HDR]_TintColor ("Tint Color", Color) = (0,0,0,0)
        _MainTex ("Main Texture", 2D) = "Black" {}
        _DispMap ("Displacement Map (RG)", 2D) = "Black" {}
        _MaskTex ("Mask(R)", 2D) = "Black" {}
        _MainSpeedX  ("Main Map Scroll Speed X", Float) = 0
        _MainSpeedY  ("Main Map Scroll Speed Y", Float) = 0
        _DispSpeedX  ("Map Scroll Speed X", Float) = 0
        _DispSpeedY  ("Map Scroll Speed Y", Float) = 0
...
//            uniform fixed _SkinDeGamma;
            ...
            uniform half _DispSpeedX;
            uniform half _DispSpeedY;
            uniform half _MainSpeedX;
            uniform half _MainSpeedY;
...
//                float3 sPc_skin = Specularskin(gloss, max(Ndothd,Ndoth), _lf, spGGX, NdotL, attenColor, Ndotv, _MatMask_var.r, Ndotl, _skinspcolor, specmncrm);
                float3 specular = CalculateSpecular(_SpecularmapRGBA_var.rgb,spGGX,gloss,attenColor,IBL,1,0,0);
...
//                half3 NdotLs = SkinLighting1(NdotL,_NdotL,shval,_Skinenv.rgb,Ndotl,NdotV,_se,scatter,Ndoth,lightColor) * _MatMask_var.r;
                half3 diffuse = CalculateDiffuseAddonf(0,NdotLq,0,attenColor,sh,_BaseRGBA_var.rgb,0,_MatMask_var.r);
//////// Emissive:
                half3 dispfx = displacementbynormalmap(i.uv3.zw,i.uv1.zw,i.uv3.xy,_DispSpeedX,_DispSpeedY,_MainSpeedX,_MainSpeedY,_DispMap,_TintColor,_MaskTex,_DispX,_DispY,_MainTex);
/// Final Color:
