Shader "TERA/Character/Heroic_Fashion_RainbowFx_Transparent" {
    Properties {
...
///// SkinColor:
                half3 baseRGBA = lerp(_BaseRGBA_var.rgb,_rainbow,0.7);
///// Lightness:
...
///////// Gloss:
				half gloss = _SpecularmapRGBA_var.a;
///// SkinColor:
                half3 baseRGBA = lerp(_BaseRGBA_var.rgb,_rainbow,0.2);
///// Lightness:
                half lightpower = ((lightColor.r*0.299)+(lightColor.g*0.587)+(lightColor.b*0.114));
                half emispower = clamp(lightpower,0.0,1.0);
                float NdotL = max(0, dot( normalDirection, lightDirection ));
...
/////// Diffuse:
                half3 diffuse = (NdotL * attenColor + 0.7 * sh) * lerp(_BaseRGBA_var.rgb,_rainbow,0.6);
/// Final Color:
