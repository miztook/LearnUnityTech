Shader "TERA/Environment/SkySphereWithStars"
{
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				v2f vert(appdata v)
				{
					...
					o.uv2.xy = o.uv1.xy + _RotSpeed.w * _Time.x;

					return o;
				}

				//star shinning pattern
				inline float unity_noise_randomValue(float2 uv)
				{
					return saturate((sin(dot(uv, float2(40, 20))) + 1) / 2);
				}
				...
				fixed4 frag(v2f i) : SV_Target
				{
					...
					fixed2 l4 = tex2D(_addTex, i.uv0.xy).rg;	//bloom, stars
					final_color += maincolor * l4.r;

					//stars
					fixed pla = unity_noise_randomValue(i.uv2.xy);
					final_color += l4.g * pla * _StarBright;
