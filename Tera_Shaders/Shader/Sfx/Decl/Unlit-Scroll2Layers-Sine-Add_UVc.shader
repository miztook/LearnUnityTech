...
Shader "TERA/Fx/Decl/Scroll 2 Layers Sine AlphaBlended" {
Properties {
...
SubShader {
	...
	v2f vert (appdata_full v)
	{
		...
		float2 uv = mul(unity_Projector, v.vertex);
		o.uv.xy = TRANSFORM_TEX(uv,_MainTex) + frac(float2(_ScrollX, _ScrollY) * _Time);
		o.uv.zw = TRANSFORM_TEX(uv,_DetailTex) + frac(float2(_Scroll2X, _Scroll2Y) * _Time);
